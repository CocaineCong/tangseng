...
def rmac(x, L=3, eps=1e-6):
    ...
    for i in range(1, L + 1):
        ...
        if i + Wd == 1:
            ...
            b = (W - wl) / (i + Wd - 1)
        cenW = torch.floor(wl2 + torch.Tensor(range(i - 1 + Wd + 1)) * b) - wl2  # center coordinates
        if i + Hd == 1:
            b = 0
        else:
            b = (H - wl) / (i + Hd - 1)
        cenH = torch.floor(wl2 + torch.Tensor(range(i - 1 + Hd + 1)) * b) - wl2  # center coordinates
...
def roi_pool(x, r_pool, L=3, eps=1e-6):
    ovr = 0.4  # desired overlap of neighboring regions
    steps = torch.Tensor([2, 3, 4, 5, 6, 7])  # possible regions for the long dimension
    ...
    for i in range(1, L + 1):
        ...
        if i + Wd == 1:
            ...
            b = (W - wl) / (i + Wd - 1)
        cenW = torch.floor(wl2 + torch.Tensor(range(i - 1 + Wd + 1)) * b).int() - wl2  # center coordinates
        if i + Hd == 1:
            b = 0
        else:
            b = (H - wl) / (i + Hd - 1)
        cenH = torch.floor(wl2 + torch.Tensor(range(i - 1 + Hd + 1)) * b).int() - wl2  # center coordinates

        for i_ in cenH.tolist():
            for j_ in cenW.tolist():
                if wl == 0:
                    continue
                vecs.append(r_pool(x.narrow(2, i_, wl).narrow(3, j_, wl)).unsqueeze(1))
